...
initialValues = dict()
initVmem = torch.linspace(-0.2,0,numSamples)
...
initialValues['G_pol']['values'] = [torch.FloatTensor([0.1])] * numSamples  # bistable
initialValues['G_dep'] = dict()
initialValues['G_dep']['cells'] = []
# initialValues['G_dep']['values'] = torch.FloatTensor([])
initialValues['G_dep']['values'] = [torch.FloatTensor([1.5])] * numSamples

circuit.initVariables(initialValues)
circuit.initParameters(initialValues)

utils = utilities.utilities()
electrodomeIndices = utils.computeDomeIndices(circuit.LatticeDims,resolution=1,mode='field')
# print("Initial Vmem:")
# print(circuit.Vmem.view(numSamples,*circuitDims))
clampIndices = electrodomeIndices
# clampFieldParameters = (clampMode,clampIndices,clampVoltage,clampDurationPercent)
circuit.simulate(clampParameters=None,numSimIters=numSimIters,saveData=True)
# print("\nFinal Vmem:")
