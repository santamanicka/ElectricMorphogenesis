...
trajectory, learned vs one representative random seed -- shows whether the SAME cells stay
dominant over time (stable identity) or different cells cycle in and out (shifting identity),
which the scalar boundary-share/velocity curves from analyze_provenance_unfolding.py couldn't
show directly.

  python visualize_provenance_blocks_unfolding.py
"""
...
conditions = [('learned', 'data/provenanceTimecourse_learned.npz'),
             (f'random (seed {RANDOM_SEED_TO_SHOW})',
              f'data/provenanceTimecourse_seed{RANDOM_SEED_TO_SHOW}_random.npz')]
...
for name, idxs in blocks.items():
    for label, path in conditions:
        ...
        for colIdx, t in enumerate(TIME_POINTS):
            row = np.asarray(d[f'pVmem_step{t}'])[idxs].mean(axis=0)
...
fig.suptitle('Top-5 contributor maps over time: learned vs random, by facial-feature block',
            fontsize=11.5, y=0.995)
plt.savefig('figures/provenanceBlocksUnfolding.png', dpi=150, bbox_inches='tight')
plt.close()
print("Saved figures/provenanceBlocksUnfolding.png")
