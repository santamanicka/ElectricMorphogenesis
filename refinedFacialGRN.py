...
2. Bioelectric signals (Ca²⁺, metabolic state) - 30% weight
...
class RefinedFacialGRN:
    ...
    - Bioelectric signals (Ca²⁺, metabolic) modulate morphogen secretion AND gene activation
    ...
    def __init__(self, grid_size, device='cpu', dtype=torch.float64):
        ...
        self.gene_params = {
            ...
            'w_initiation': torch.tensor(1.0, device=device, dtype=dtype),   # Morphogen × Bio_gate strength
            'w_maintenance': torch.tensor(0.0, device=device, dtype=dtype),  # Pure AND logic initially
    ...
    def initialize_morphogen_sources(self):
        ...
        y_coords = torch.linspace(0.0, 1.0, gs, device=self.device, dtype=self.dtype).view(gs, 1).expand(gs, gs)
        x_coords = torch.linspace(0.0, 1.0, gs, device=self.device, dtype=self.dtype).view(1, gs).expand(gs, gs)
        dist_from_midline = torch.abs(x_coords - 0.5)

        # Morphogen source patterns adapted from FacialGRN.html (40x40 → 11x11)
        # Following FacialGRN.html lines 309-326 exactly

        # SHH: High at midline, decays laterally (inverted V)
        # SHARPER decay for 11x11 grid to create spatial specificity
        shhAnteriorBoost = 1.0 - y_coords * 0.5  # Stronger in anterior
        shh_decay_length = 0.8 / gs  # NARROWER than scaled (was 1.4/gs)
        self.shh_source = shhAnteriorBoost * torch.exp(-dist_from_midline / shh_decay_length)

        # FGF8: High laterally, low at midline (V-shaped)
        # MATCHING sharp spatial profile to SHH for complementarity
        fgf8AnteriorBoost = 1.0 - y_coords * 0.5  # Stronger in anterior
        fgf8_decay_length = 0.3 / gs  # MUCH SHARPER onset than SHH (was 0.8/gs)
        self.fgf8_source = fgf8AnteriorBoost * (1.0 - torch.exp(-dist_from_midline / fgf8_decay_length))

        # EDN1: Linear gradient from anterior (0) to posterior (1)
        # FacialGRN.html line 324-325: params.edn1Strength * 0.8 * normalizedY
        self.edn1_source = 0.8 * y_coords  # Simple A→P gradient
    ...
    def gene_dynamics(self, morph_signal, bio_gate, current_gene, bioelectric_signals, params, dt):
        ...
        if bioelectric_signals is None:
            # Autonomous: simple relaxation dynamics (FacialGRN.html line 426)
            dgene_dt = k_on * (morph_signal - current_gene) - k_off * current_gene
        else:
    ...
    def update_morphogens(self, bioelectric_signals=None):
        """
        Update morphogen concentrations via diffusion-degradation.
        Morphogen SECRETION is modulated by bioelectric signals.

        Args:
            bioelectric_signals: dict with keys 'Ca', 'metabolic' (optional)
        """
        ...
        k_deg_fgf8 = k_deg * 10.0  # 10× higher degradation for FGF8 (0.5 instead of 0.05)
        ...
        # EDN1: Simple maintenance (FacialGRN.html line 385-386)
        # "Edn1 doesn't diffuse much - just maintain posterior expression"
        self.grid['edn1'] = self.grid['edn1']  # Static gradient
    ...
    def update(self, bioelectric_signals=None):
        ...
        Args:
            bioelectric_signals: dict with 'Ca' and 'metabolic' keys
        """
        self.update_morphogens(bioelectric_signals)
    ...
    def simulate(self, num_steps, bioelectric_signals=None):
        ...
        Args:
            ...
            bioelectric_signals: dict with 'Ca' and 'metabolic' (can be time-varying)
        """
