"""Visualize the current best-so-far Vmem pattern in each tissue-band depth-restricted static-hold
class (tissueBandGpolTwoFoldSymmetry, screen4/weight1000) -- these runs are still training, so this
is an interim snapshot, not a final result. Same layout/convention as plotBestFaceAllClasses.py:
rows are classes, columns are target / prepattern / correlation-loss best / globalsum-loss best.
target/actual are read straight from each checkpoint's own stored targetVmem/actualVmem -- nothing
re-simulated there. The prepattern (Vmem at clampEndIter+1, the moment the clamp releases and free
evolution takes over) is not stored anywhere, so it requires a short replay of just the clamp phase --
cheap and low chaos-risk, same reasoning as compareTrainedPrepatternCommitment.py's own prepattern read.
"""
import torch
import matplotlib
matplotlib.use('Agg')
...
classes = [
    ('Gpol-only, S11/str0.25', [1520,1521,1522,1523,1524,1525], [1526,1527,1528,1529,1530,1531]),
    ('Gpol+Vmem, S11/str0.25', [1532,1533,1534,1535,1536,1537], [1538,1539,1540,1541,1542,1543]),
]
...
fig.suptitle('Tissue-band static hold, best so far (30x30, screen11/strength0.25, depth3/300-iter hold) -- runs still in progress', fontsize=12)
fig.tight_layout()
fig.savefig('figures/tissueBandInterim.png', dpi=140, bbox_inches='tight')
print('wrote figures/tissueBandInterim.png')
