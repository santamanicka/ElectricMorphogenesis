"""No clamp, symmetry-breaking noise only, fixed weight=700, across fieldScreenSize -- does the
tissue's own unforced dynamics change with action range at the weight already chosen for its
pre-pattern match to 11x11?

Same protocol as analyzeAutonomousByWeight.py (noiseStd volts of Vmem noise, no clamp, full
horizon), with fieldScreenSize swept instead of weight, weight held fixed.
"""
import argparse
...
parser.add_argument('--output', default='./figures/autonomousByScreen_weight700.png')
...
figure.suptitle(f'30x30, no clamp, weight={args.weight:.0f}: does screen size change the autonomous dynamics?',
                 fontsize=11)
