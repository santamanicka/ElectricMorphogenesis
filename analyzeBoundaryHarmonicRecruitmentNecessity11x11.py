...
Two measurements, both on the darkening events of the settled second rise:
  ...
  reach   The drive is not external to the tissue: the field every cell reads is generated by every cell's
          voltage. So "the neighbour was not needed" does not mean no other cell was. This measures how far
          from a perturbed cell its knock-on changes actually land, in lattice steps, against the null of all
          interior cells at that distance.
"""
import argparse
import json
...
# ------------------------------------------------------------------ how far one cell's state reaches
def latticeDistance(a, b):
    return abs(a // boundary.latticeCols - b // boundary.latticeCols) + abs(a % boundary.latticeCols - b % boundary.latticeCols)


reach = {}
for kind in ('block', 'force'):
    observed, available = {}, {}
    for index, (label, cell) in enumerate(zip(kinds, cells)):
        if str(label) != kind:
            continue
        cell = int(cell)
        for other in interior:
            if other == cell:
                continue
            step = latticeDistance(cell, other)
            available[step] = available.get(step, 0) + 1
            if darkAtBest[index][other] != baseline[other]:
                observed[step] = observed.get(step, 0) + 1
    total = sum(observed.values())
    screen = int(boundary.loadCheckpoint(1888)['fieldParameters']['fieldScreenSize'])
    reach[kind] = dict(
        runs=sum(1 for label in kinds if str(label) == kind), changed=total, screenSize=screen,
        byDistance=[dict(steps=step, changed=observed.get(step, 0), available=available[step])
                    for step in sorted(available)],
        maxDistance=max(observed) if observed else 0,
        beyondScreen=sum(v for step, v in observed.items() if step > screen),
        beyondScreenShare=round(sum(v for step, v in observed.items() if step > screen) / total, 3) if total else None)
...
for kind, entry in reach.items():
    print(f"{kind}: {entry['changed']} knock-on changes, reaching {entry['maxDistance']} lattice steps; "
          f"{entry['beyondScreen']} ({100 * entry['beyondScreenShare']:.0f}%) beyond the "
          f"{entry['screenSize']}-cell field screen", flush=True)
