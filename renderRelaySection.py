...
Every other section is hidden. The movie can be set to a slider position, to the shares view of its chart and to
per-frame arrow scaling before the screenshot, and the page can be forced into the dark theme.

    python3 renderRelaySection.py figures/boundaryHarmonicSwitchRule.html out.png [--height 6400] [--dark]
                                  [--position 12] [--shares] [--rescale]
...
parser.add_argument('--position', type=int, default=None, help='movie slider position: 0 is the initial state, the last is the final state')
...
if args.position is not None or args.shares or args.rescale:
    steps = ''
    if args.rescale:
    ...
    if args.position is not None:
        steps += 'const s=document.getElementById("movieSlider");s.value=%d;s.dispatchEvent(new Event("input"));' % args.position
