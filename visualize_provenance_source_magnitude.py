"""
Full-bulk version of visualize_provenance_source_magnitude_bulkFeatureOnly.py: the same
magnitude-weighted dominance map (hue = winning boundary group via forced argmax over the 5 pooled
groups, opacity = the target's own |Vmem-V_th| on a shared scale across both conditions), but now
covering all 81 bulk cells instead of only the 14 facial-feature cells. Facial-feature cells are
outlined for reference, tying back to the restricted version.

  python visualize_provenance_source_magnitude.py
"""
...
for label, path in conditions:
    ...
    allWinner[label] = {int(t): groupNames[np.argmax(groupTotals[int(t)])] for t in bulkIndices}
...
for power, schemeName in [(1, 'magnitude'), (2, 'magnitudeSquared')]:
    ...
    fig.suptitle(f'{schemeName}-weighted dominance map, all 81 bulk cells: hue = winning boundary group '
                f'(forced\nargmax), opacity = target\'s own |Vmem-V_th| ({schemeName}), SAME scale both '
                'panels\n(circles mark the 14 facial-feature cells for reference)',
                fontsize=10.3, y=1.08)
    outPath = f'figures/provenanceSourceMagnitude_{schemeName}.png'
...
featVals = {label: [allDev[label][t] for t in featureIndices] for label in allDev}
nonFeatVals = {label: [allDev[label][t] for t in bulkIndices if int(t) not in featureIndices] for label in allDev}
print("\nfeature cells vs. rest of the bulk:")
for label in allDev:
    print(f"  {label}: feature-cell mean {np.mean(featVals[label]):.1f} mV, "
         f"rest-of-bulk mean {np.mean(nonFeatVals[label]):.1f} mV")
