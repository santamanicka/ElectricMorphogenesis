"""PolyPatterning_Sim.md Sec 11.12: Gpol-only vs. Gpol+Vmem band-limited static hold, both at their
best-performing setting (screen11/strength0.25, depth 3, 300-iteration hold) -- final, fully-trained
checkpoints, not an interim snapshot. Same layout/convention as plotBestFaceAllClasses.py: rows are
classes, columns are target / clamp iter 1 / prepattern / correlation-loss best / globalsum-loss
best. target/actual are read straight from each checkpoint's own stored targetVmem/actualVmem --
nothing re-simulated there. The prepattern and clamp-iter-1 columns are not stored anywhere, so they
require a short replay of just the clamp phase -- cheap and low chaos-risk, same reasoning as
...
fig.suptitle('Tissue-band static hold: Gpol-only vs. Gpol+Vmem (30x30, screen11/strength0.25, depth3/300-iter hold)', fontsize=12)
fig.tight_layout()
fig.savefig('figures/tissueBandGpolVmemComparison_screen11str025.png', dpi=140, bbox_inches='tight')
print('wrote figures/tissueBandGpolVmemComparison_screen11str025.png')
