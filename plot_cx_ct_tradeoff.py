"""Plot the complexity-controllability trade-off across field action range, for both lattices."""
...
parser.add_argument('--inputs', type=str,
                    default="[('11x11','data/cxCt11x11Dense_metricEvaluation.npz'),"
                            "('30x30','data/cxCt30x30Dense_metricEvaluation.npz')]")
parser.add_argument('--output', type=str, default='data/cxCtTradeoff.png')
...
for column, (label, path) in enumerate(inputs):
    ...
    axis.plot(reach, get('CT_variance'), 's-', color='crimson', label='CT  controlled variance')
    ...
    axis.plot(reach, get('fair_CT_perCell'), 's-', color='crimson', label='CT  arithmetic mean')
    axis.plot(reach, get('geo_CX_perCell'), 'o--', color='steelblue', alpha=0.55, label='CX  geometric mean')
    axis.plot(reach, get('geo_CT_perCell'), 's--', color='crimson', alpha=0.55, label='CT  geometric mean')
    ...
    axis.plot(reach, get('CT_fraction'), 'o-', color='darkorange', label='whole interior')
    axis.plot(reach, get('fair_CT_fraction'), 's--', color='seagreen',
    ...
    # 4. The trade-off itself, as a path through the CX-CT plane.
    axis = figure.add_subplot(grid[4, column])
    arithCx, arithControl = get('fair_CX_perCell'), get('fair_CT_perCell')
    cx, control = get('geo_CX_perCell'), get('geo_CT_perCell')
    axis.plot(arithCx, arithControl, '-', color='0.82', linewidth=1, zorder=0)
    axis.scatter(arithCx, arithControl, c='0.82', s=26, zorder=0, label='arithmetic mean')
    axis.plot(cx, control, '-', color='0.6', linewidth=1, zorder=1)
    onFront = [i for i in range(len(cx))
               if not any(cx[j] >= cx[i] and control[j] >= control[i] and
                          (cx[j] > cx[i] or control[j] > control[i]) for j in range(len(cx)))]
    axis.scatter(cx[onFront], control[onFront], s=230, facecolor='none', edgecolor='crimson',
                 linewidth=1.8, zorder=3, label='Pareto frontier')
    scatter = axis.scatter(cx, control, c=reach, cmap='viridis', s=70, zorder=2, edgecolor='k', linewidth=0.4)
    axis.legend(fontsize=8, loc='lower right')
    for r, x, y in zip(reach, cx, control):
    ...
    axis.set_ylabel('CT  (geometric mean\nover shells, mV$^2$)')
    ...
    axis.plot(reach, get('CT_bits'), 'o-', color='rebeccapurple', label='whole interior')
    axis.plot(reach, get('fair_CT_bits'), 's--', color='teal',
              label='depth-fair (arithmetic mean)')
    ...
    profile = np.array([records[r]['profile_CT_fraction'] for r in ranges], dtype=float)
